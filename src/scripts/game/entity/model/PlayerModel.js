...
export default class PlayerModel extends EntityModel {
    constructor() {
        super();

    }
    build(name, label, stats, modifiers, graphicsData, config) {
        ...
        this.movementSpeedModifier = this.modifiers.movementSpeedModifier;
        ...
        this.baseHP = this.level * (30 / this.baseHPModifier);
    ...
    log() {
        ...
        console.log('movementSpeed,', Math.floor(this.movementSpeed));
    ...
    levelUp() {
        ...
        this.movementSpeed += (this.movementSpeed * this.movementSpeed + this.movementSpeed + 3) / 4 * this.movementSpeedModifier;
        ...
        this.movementSpeedModifier -= 0.0005;
        ...
        if (this.movementSpeedModifier <= 0.001) {
            this.movementSpeedModifier = 0.001;
        }
    ...
    getDemage(type = 'physical', weaponPower = 1, spellPower = 1) {
        var damageMultiplierCritical = Math.random() < this.critialChance ? 0.5 : 2;
        var damageMultiplier = (Math.random() / 2) + 1;
        var demage = 0;
        if (type === 'physical') {
            demage = this.battlePower * this.level + ((this.level * this.attack * weaponPower) / 256) * 3 / 2;
        } else if (type === 'magical') {
            demage = spellPower * 4 + (this.level * this.magicPower * weaponPower / 32);
        }
    ...
    getSpeedW(type) {
        ...
        if(this.weapon){
            return currentSpeed * this.weapon.speedMultiplier;
        }else{
            return currentSpeed
        }
    }
