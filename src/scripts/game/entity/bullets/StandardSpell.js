...
export default class StandardSpell extends PIXI.Container {
    constructor() {
        super();
        this.speed = 120
        this.resetVelocity();
    ...
    reset(texture) {
        this.entitySprite.texture = PIXI.Texture.from(texture);
        this.shotting = false;
    }
    ...
    update(delta) {
        if (!this.shotting) {
            ...
            this.attackAngle = Math.atan2(this.y - this.target.y, this.x - this.target.x);
            this.entitySprite.rotation = this.attackAngle - Math.PI / 2
            ...
            if (utils.distance(this.x, this.y, this.target.x, this.target.y) < this.target.radius) {
                // if (this.target.actionType != StandardEntity.ACTIONS.DEAD) {
                    // this.collide(this.target);
                    this.onCollide.dispatch(this.entity, this.target, this.type)
                    // return;
                // }
                this.onDestroy.dispatch(this);
            }
